...
import jakarta.validation.Valid; //Triggers validation on theobject
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity; //Allows control over HTTP status codes and headers
...
public class TaskController {
    ...
    @GetMapping //Handles HTTP get requests (returning tasks)
    public ResponseEntity<List<Task>> getAllTasks() {
        List<Task> tasks = taskService.getAllTasks();
        return ResponseEntity.ok(tasks);
    }
    
    @GetMapping("/{id}") //Handles HTTP get request by ID
    ...
    @PostMapping //Handles HTTP post requests (Creating tasks)
    public ResponseEntity<Task> createTask(@Valid @RequestBody Task task) {
        Task createdTask = taskService.createTask(task);
        return ResponseEntity.status(HttpStatus.CREATED).body(createdTask);
    }
    
    @PutMapping("/{id}") //Handles HTTP put requests (Update task by ID)
    ...
    @DeleteMapping("/{id}") //Handles HTTP delete requests (Deletes task by ID)
